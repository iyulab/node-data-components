...
- **포커스 테두리의 폴백 리터럴 2곳이 낡아 있었다** — `@iyulab/components` 시트가
  상태 테두리를 한 단 진하게 옮기면서(다크 대비 미달 해소) 여기 구워 둔 값이 어긋났다.
  ⚠**토큰 시트를 로드한 환경에서는 드러나지 않는다** — 폴백이 평가되지 않기 때문이다.
  깨지는 곳은 시트 없이 이 패키지를 쓰는 소비자의 화면이다.
- **검색 입력의 포커스 링이 팔레트에 없는 색이었다** — `rgba(59, 130, 246, .2)`
  (Tailwind blue-500)를 굽고 있어 브랜드를 덮어도 따라오지 않았고 다크에서도 같은 값이었다.
  역할 토큰에서 `color-mix()` 로 파생하도록 고쳤다.
