...
⚠**인쇄에는 스크롤이 없습니다** — 호스트에 준 높이는 인쇄에서도 적용돼 콘텐츠 영역이 그 높이만큼만 찍힙니다(실측: 300px 표가 행 영역 2067px 중 283px). 인쇄할 수 있는 화면이면 높이를 `@media screen { … }` 안에서 주십시오 — 인쇄에서는 내용 높이로 나옵니다. `u-rich-table` 도 같습니다.
